  ...
  if (!data.length) {
    return (
      <div className="bg-black border border-white/20 rounded-xl p-6">
        ...
        <div
          className={`bg-gray-900/50 rounded-lg flex items-center justify-center border border-white/10`}
          style={{ height }}
        >
        ...
        <div className="flex items-end justify-between h-full space-x-2">
          {data.map((item, index) => {
            const barHeight = maxValue > 0
              ? (item[yKey] / maxValue) * chartHeight
              : 0;
        ...
        <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t border-white/10">
          ...
          <div className="text-center">
            <div className="text-lg font-semibold text-green-400">
              {formatValue(
                data.reduce((sum, item) => sum + item[yKey], 0) / data.length,
              )}
            </div>
