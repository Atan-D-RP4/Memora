...
const AddTopicModal = ({ isOpen, onClose, onSubmit, loading = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({
        submit: error.message || "Failed to create topic. Please try again.",
      });
    }
  };
            ...
            {formData.externalLinks.length > 0 && (
              <div className="space-y-2">
                ...
                {formData.externalLinks.map((link, index) => (
                  <div
                    key={index}
                    className="flex items-center justify-between bg-black border border-white/20 rounded-lg p-3"
                  >
