...
const Analytics = () => {
  ...
  const loadAnalyticsData = async () => {
    ...
    try {
      ...
      const totalTopics = (topicsResponse.success && topicsResponse.topics)
      ...
      const completionRate = calculateCompletionRate(
        (topicsResponse.success && topicsResponse.topics)
          ? topicsResponse.topics
          : [],
      );

      // Process topic performance data
      const topicPerformance = processTopicPerformance(
        (topicsResponse.success && topicsResponse.topics)
          ? topicsResponse.topics
          : [],
      );
      const difficultyBreakdown = processDifficultyBreakdown(
        (topicsResponse.success && topicsResponse.topics)
          ? topicsResponse.topics
          : [],
      );
  ...
  const calculateCompletionRate = (topics) => {
    ...
    const completedTopics =
      topics.filter((topic) => topic.reviewCount > 0).length;
  ...
  const generateWeeklyStats = (sessions) => {
    ...
    return {
      ...
      averageSession: thisWeek.length > 0
        ? thisWeek.reduce((sum, session) => sum + (session.duration || 0), 0) /
          thisWeek.length
        : 0,
    };
  };
  ...
  const generateRecentActivity = (topics) => {
    ...
    return topics
      .filter((topic) =>
        topic && topic.lastReviewDate && topic._id && topic.title
      )
            ...
            : (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-black border border-white/20 rounded-xl p-6">
                    ...
                    <div className="space-y-4">
                      {analyticsData.studyPatterns.dailyActivity.map((
                        day,
                        index,
                      ) => (
                        <div
                        ...
                        >
                          <div className="flex items-center space-x-4 flex-1">
                            <span className="text-sm text-gray-400 w-10 font-medium">
                              {day.day}
                            </span>
                            <div className="flex-1 bg-gray-800 rounded-full h-3 hover:bg-gray-700 transition-colors duration-200">
                              <motion.div
                                ...
                                style={{
                                  width: `${
                                    Math.min((day.sessions / 5) * 100, 100)
                                  }%`,
                                }}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {/* Daily Usage Tracker */}
                  <DailyUsageTracker
                    data={analyticsData.studyPatterns.dailyActivity}
                  />
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                      <div className="text-center p-3 rounded-lg hover:bg-blue-500/10 transition-colors duration-200">
                        <motion.div
                          ...
                          transition={{ duration: 0.2 }}
                        >
                          {analyticsData.studyPatterns.dailyActivity.reduce(
                      ...
                      <div className="text-center p-3 rounded-lg hover:bg-orange-500/10 transition-colors duration-200">
                        <motion.div
                          ...
                          transition={{ duration: 0.2 }}
                        >
                          {analyticsData.studyPatterns.dailyActivity.filter(
                            (day) => day.minutes > 0,
                          ).length}
                        </motion.div>
                        ...
                        {analyticsData.studyPatterns.dailyActivity.map(
                          (day, index) => {
                            const maxMinutes = Math.max(
                              ...analyticsData.studyPatterns.dailyActivity
                                .map((d) => d.minutes),
                            ) || 60;
                            const height = day.minutes > 0
                              ? Math.max((day.minutes / maxMinutes) * 100, 10)
                              : 5;
                        ...
                        <div className="bg-white/5 rounded-lg p-3 hover:bg-white/10 transition-colors duration-200">
                          ...
                          >
                            {(() => {
                              ...
                              const totalSessions = analyticsData.studyPatterns
                              ...
                              const activeDays =
                                analyticsData.studyPatterns.dailyActivity
                                  .filter((day) => day.sessions > 0).length;
                              const consistencyScore = activeDays > 0
                                ? (activeDays / 7) * 100
                                : 0;
                              const sessionScore = Math.min(
                                totalSessions * 10,
                                100,
                              ); // 10% per session, max 100%
                              return Math.round(
                                (consistencyScore + sessionScore) / 2,
                              );
                            })()}%
                          </motion.div>
                          <div className="text-xs text-gray-500">
