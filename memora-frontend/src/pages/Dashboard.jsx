...
const Dashboard = () => {
  ...
  const motivationalQuotes = [
    ...
    {
      text: "Your memory is a muscle. Use it or lose it.",
      author: "Anonymous",
    },
  ];
  ...
  const fetchAllTopicsAndCalculate = async () => {
    try {
      const [dueTopicsData, upcomingTopicsData, workloadData] = await Promise
        .all([
          fetchDueTopics(),
          fetchUpcomingTopics(),
          fetchWorkloadData(),
  ...
  const calculateNextSevenDays = (topics, workloadData = []) => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      next7Days.push({
        ...
        difficultyAdjustedLoad: workloadDay
          ? workloadDay.difficultyAdjustedLoad
          : actualCount * 3,
      });
    }
  ...
  const handlePreventCrowding = async (targetDate) => {
    try {
      ...
      if (response.success && response.redistributed) {
        showToast(
          `${response.count} topics redistributed based on difficulty levels`,
          "success",
        );
  ...
  const handleMoveOverdueTopics = async (silent = false) => {
    try {
      ...
      if (response.success && response.moved > 0) {
        if (!silent) {
          showToast(
            `${response.moved} overdue topics moved to today`,
            "success",
          );
        }
  ...
  const handleTopicReview = async (topicId, quality = 3) => {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        setToast({
          show: true,
          message: `✅ Topic completed! Next review: ${
            new Date(response.topic.nextReviewDate).toLocaleDateString("en-GB")
          }`,
          type: "success",
        });
      } else {
  ...
  const handleTopicDelete = async (topicId) => {
    const topic = dueTopics.find((t) => t._id === topicId) ||
      upcomingTopics.find((t) => t._id === topicId);
  ...
  const handleFastReview = async (topicId) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setToast({
          show: true,
          message: `⚡ Fast review completed! Next review: ${
            new Date(response.topic.nextReviewDate).toLocaleDateString("en-GB")
          }`,
          type: "success",
        });
  ...
  const handleTopicClick = (topic) => {
    ...
    const daysUntilReview = Math.ceil(
      (nextReviewDate - currentDate) / (1000 * 60 * 60 * 24),
    );
    ...
    for (let i = 0; i < 5; i++) {
      ...
      reviewDate = new Date(
        reviewDate.getTime() + currentInterval * 24 * 60 * 60 * 1000,
      );
    }

    const reviewDatesText = futureReviews
      .map((date, index) =>
        `${index + 1}. ${date.toLocaleDateString("en-GB")} (${
          index === 0
            ? daysUntilReview
            : Math.ceil((date - currentDate) / (1000 * 60 * 60 * 24))
        } days)`
      )
      .join("\n");
      ...
      if (response.success) {
        ...
        if (response.isNewRecord) {
          ...
          setToast({
            show: true,
            message:
              `🔥 ${response.currentStreak} day streak! Amazing consistency!`,
            type: "streak",
          });
        }
      }
  ...
  const quickActions = [
    {
      icon: Plus,
      label: "Add Topic",
      action: () => setShowAddTopicModal(true),
      primary: true,
    },
    {
      icon: Zap,
    ...
    <div className="bg-black text-white min-h-screen flex">
      ...
      >
        ...
        <div className="flex-1 p-3 sm:p-6 transition-all duration-300">
          <div
            className={`grid grid-cols-1 gap-4 sm:gap-6 xl:grid-cols-4 ${
              sidebarCollapsed ? "mx-24" : ""
            }`}
          >
                  ...
                  <div
                    ...
                    onWheel={(e) => {
                      ...
                      const isPageNearBottom =
                        pageScrollTop + windowHeight >= pageHeight - 50; // 50px threshold
                                  ...
                                  <div className="flex-1 pr-4">
                                    ...
                                    <div className="flex items-center space-x-4 text-xs text-gray-500">
                                      ...
                                      <div className="flex items-center space-x-1">
                                        ...
                                        <span>
                                          Due: {new Date(topic.nextReviewDate)
                ...
                >
                  ...
                  <div
                    ...
                    onWheel={(e) => {
                      ...
                      const isPageAtAbsoluteBottom =
                        Math.abs((pageScrollTop + windowHeight) - pageHeight) <=
                          1;

                      // Additional check - ensure we can't scroll down anymore
                      const canScrollDown =
                        pageScrollTop + windowHeight < pageHeight;
                    ...
                    {loadingUpcoming
                      ...
                      ? (
                        <div className="flex flex-col items-center justify-center flex-1 text-center">
                          ...
                          <p className="text-gray-400">
                            Add topics and study to see your schedule.
                          </p>
                        </div>
                      )
                      : (
                    ...
                    {upcomingTopics.length > 4 && (
                      <div className="mt-4 pt-4 border-t border-white/10 flex-shrink-0">
                        ...
                        >
                          {showAllUpcoming
                            ? "Show Less"
                            : `View All ${upcomingTopics.length} Upcoming Topics`}
                        </button>
                      </div>
                    )}
                  </div>
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-2">
                            <div
                              ...
                              title={`${day.topics} topics on ${day.day} - ${day.crowdingLevel} load (Avg difficulty: ${
                                day.averageDifficulty?.toFixed(1) || "N/A"
                              })${day.isCrowded ? " - CROWDED!" : ""}`}
                            >
                            </div>
                            {day.isCrowded && (
                              <button
                                onClick={() =>
                                  handlePreventCrowding(new Date(day.date))}
                    ...
                    : (
                      <div className="text-center py-4">
                        ...
                        <p className="text-gray-500 text-xs">
                          Add topics and study to see your upcoming schedule
                        </p>
                      </div>
                    )}
                </div>
