...
const FocusMode = () => {
  ...
  useEffect(() => {
    ...
    if (!isRunning && !isPaused) {
      ...
      if (settings.timerMode === "countdown") {
        const time = settings.studyMethod === "pomodoro"
          ? 25 * 60
          : settings.customMinutes * 60;
  ...
  const savePreset = (name, autoLoad = true) => {
    ...
    localStorage.setItem(
      getUserStorageKey("focusModePresets"),
  ...
  const loadPreset = (preset, showToastNotification = false) => {
    ...
    if (isRunning) {
      showToast(
        `Cannot load preset while timer is running. Pause or stop the timer first.`,
        "error",
      );
      return;
    }
    ...
    if (preset.timerMode === "countdown") {
      const time = preset.studyMethod === "pomodoro"
        ? 25 * 60
        : preset.customMinutes * 60;
      setGlobalInitialTime(time);
    }
    ...
    if (studyMethod === "pomodoro") {
      if (currentPhase === "study") {
        ...
        if (currentSession < pomodoroSessions) {
          ...
          addSessionEvent("next_session_ready", {
  ...
  const adjustTime = (minutes) => {
    ...
    if (isRunning) {
      showToast(
        `Cannot adjust time while timer is running. Pause the timer first.`,
        "error",
      );
      return;
    }
    ...
    if (timerMode === "countdown") {
      ...
      showToast(
        `Timer ${minutes > 0 ? "increased" : "decreased"} by ${
          Math.abs(minutes)
  ...
  const changeStudyMethod = (method) => {
    ...
    if (isRunning) {
      showToast(
        `Cannot change study method while timer is running. Pause or stop the timer first.`,
        "error",
      );
      return;
    }
  ...
  const changeTimerMode = (mode) => {
    ...
    if (isRunning) {
      showToast(
        `Cannot change timer mode while timer is running. Pause or stop the timer first.`,
        "error",
      );
      return;
    }
    ...
    if (mode === "countdown") {
      ...
      const time = studyMethod === "continuous"
        ? customMinutes * 60
        : method.studyTime;
      setGlobalInitialTime(time);
    }
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (event) => {
      ...
      if (
        !showSettings && !showHistory && !showPresetDialog &&
        !showPresetsManager &&
        (event.key === "F11" || (event.key === "f" && event.ctrlKey))
      ) {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col">
      ...
      <div className="sm:hidden portrait:flex hidden fixed inset-0 bg-black z-50 items-center justify-center p-6">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-4">
            Focus Mode works best in landscape orientation
          </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
              ...
              {studyMethod === "pomodoro" && (
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    {timerMode === "countdown"
                      ? "How many 25-minute study sessions"
                      : "Track sessions in stopwatch mode"}
                  </p>
                </div>
              )}
              ...
              <div className="flex space-x-3">
                <button
                  onClick={() => {
                    if (isRunning) {
                      showToast(
                        `Cannot save preset while timer is running. Pause or stop the timer first.`,
                        "error",
                      );
                      return;
                    }
                ...
                <button
                  onClick={() => {
                    if (isRunning) {
                      showToast(
                        `Cannot start new session while timer is running. Pause or stop the timer first.`,
                        "error",
                      );
                      return;
                    }
          ...
          <div className="bg-black border border-white/20 rounded-xl p-6 max-w-sm w-full mx-4 shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="mb-6 p-3 bg-white/5 border border-white/10 rounded-lg">
              ...
              <div className="text-white text-sm">
                <div>
                  {timerMode === "countdown" ? "⏱️ Timer" : "⏰ Stopwatch"} •
                  {" "}
                  {studyMethod === "pomodoro" ? "Pomodoro" : "Continuous"}
                </div>
                {studyMethod === "pomodoro" && (
        ...
        >
          <div className="bg-black border border-white/20 rounded-xl p-6 max-w-lg w-full mx-4 shadow-2xl max-h-[80vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
              ...
              {savedPresets.length === 0
                ? (
                  <div className="text-center text-gray-400 py-8">
                    ...
                    <p className="text-sm">
                      Create presets in Settings to save your favorite
                      configurations
                    </p>
                  </div>
                )
                            ...
                            <div className="text-sm text-gray-400 flex items-center space-x-2 mt-1">
                              ...
                              {preset.studyMethod === "pomodoro" && (
                                <>
                                  <span>•</span>
                                  <span>
                                    {preset.pomodoroSessions} sessions
                                  </span>
                                </>
                              )}
                              {preset.studyMethod === "continuous" &&
                                preset.timerMode === "countdown" && (
                                <>
                                  <span>•</span>
            ...
            <div className="flex justify-between items-center mt-6 pt-4 border-t border-white/10">
              <div className="text-xs text-gray-400">
                {savedPresets.length}{" "}
                preset{savedPresets.length !== 1 ? "s" : ""} saved
              </div>
              <button
                onClick={() => setShowPresetsManager(false)}
              ...
              {sessionHistory.length === 0
                ? (
                  <div className="text-center text-gray-400 py-8">
                    ...
                    <p>No sessions yet</p>
                    <p className="text-sm">
                      Complete a timer session to see history
                    </p>
                  </div>
                )
                : (
                          ...
                          <div className="mt-3 pt-3 border-t border-white/10">
                            <div className="text-xs text-gray-300 space-y-1">
                              ...
                              {session.mode === "countdown" && (
                                <div>
                                  Time: {Math.floor(
                                    session.initialTime / 60,
                                  )}:{(session.initialTime % 60).toString()
                                    .padStart(2, "0")} → {Math.floor(
                                      session.finalTime / 60,
                                    )}:{(session.finalTime % 60).toString()
                                    .padStart(2, "0")}
                                </div>
                              )}
      ...
      {/* Main Timer Display */}
      <div
        className={`flex-1 flex flex-col items-center px-3 sm:px-6 ${
      ...
      >
        ...
        <div
          className={`${
            isFullscreen ? "mb-1 sm:mb-2" : "mb-2 sm:mb-4"
          } flex items-center justify-center space-x-2 sm:space-x-3`}
        >
          <div
            className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-full font-medium ${
        ...
        <div
          className={`${
            isFullscreen ? "mb-4 sm:mb-8 px-2 sm:px-4" : "mb-6 sm:mb-12"
          }`}
        >
          <div className="text-center">
            <div
              className={`font-mono font-bold text-white leading-none select-none ${
                isFullscreen
        ...
        <div
          className={`flex items-center ${
            isFullscreen ? "space-x-3 sm:space-x-4" : "space-x-3 sm:space-x-6"
          }`}
        >
          {!isRunning
            ? (
          ...
          <div
            ...
            title={isRunning || timerMode !== "countdown"
              ? "Time adjustment disabled"
              : "Click top: +1 min, Click bottom: -1 min"}
          >
            ...
            {/* Center Divider Line */}
            <div className="absolute top-1/2 left-2 right-2 h-px bg-white/30 transform -translate-y-px">
            </div>
          </div>
        </div>
