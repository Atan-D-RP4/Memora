...
journalSchema.pre("save", function (next) {
  ...
  if (this.content) {
    this.wordCount = this.content.trim().split(/\s+/).filter((word) =>
      word.length > 0
    ).length;
  }
...
journalSchema.statics.getEntriesInRange = function (
  userId,
  startDate,
  endDate,
) {
...
journalSchema.statics.generateWeeklySummaryText = async function (
  userId,
  weekStartDate,
) {
  ...
  summary += `**Period:** ${entries[entries.length - 1].dateString} to ${
  ...
  entries.forEach((entry) => {
    ...
    if (firstLine && firstLine.length > 10) {
      summary += `- **${entry.dateString}:** ${firstLine.substring(0, 100)}${
...
journalSchema.statics.generateMonthlySummaryText = async function (
  userId,
  year,
  month,
) {
  ...
  Object.keys(weeks).sort().forEach((weekKey) => {
    ...
    summary +=
      `- **Week of ${weekKey}:** ${weekEntries.length} entries, ${weekWords} words\n`;
  });
