...
topicSchema.statics.getUpcomingTopics = function (
  userId,
  days = 7,
  limit = 20,
) {
...
topicSchema.statics.getDailyTopicCounts = function (
  userId,
  startDate,
  endDate,
) {
...
topicSchema.statics.getCrowdingThresholds = function (averageDifficulty) {
  ...
  const multiplier = difficultyMultipliers[Math.round(averageDifficulty)] ||
    1.0;
...
topicSchema.statics.getCrowdingLevel = function (
  topicCount,
  averageDifficulty,
) {
...
topicSchema.statics.preventCrowding = async function (userId, targetDate) {
  ...
  const dailyCounts = await this.getDailyTopicCounts(
    userId,
    startDate,
    endDate,
  );
  ...
  const crowdedDays = dailyCounts.filter((day) => {
    const crowdingInfo = this.getCrowdingLevel(
      day.count,
...
topicSchema.statics.findBestAlternativeDate = async function (
  userId,
  originalDate,
  dailyCounts,
) {
  ...
  for (let i = -3; i <= 7; i++) {
    ...
    const currentAvgDifficulty = existingDay
      ? existingDay.averageDifficulty
      : 3; // Default to medium
