...
revisionHistorySchema.statics.getTopicPerformance = function (
  topicId,
  limit = 50,
) {
  return this.find({ topicId })
    ...
    .limit(limit)
    .select(
      "quality wasCorrect responseTime createdAt intervalAfter easeFactorAfter",
    );
};

// Static method to get performance by time of day
revisionHistorySchema.statics.getPerformanceByTimeOfDay = function (
  userId,
  days = 30,
) {
  const startDate = new Date();
...
const RevisionHistory = mongoose.model(
  "RevisionHistory",
