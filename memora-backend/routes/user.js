...
router.get("/memscore/history", authenticateToken, async (req, res) => {
  try {
    ...
    const chartData = await MemScoreHistory.getChartData(
      userId,
      parseInt(days),
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      const { memoryGame, tileRecall, processingSpeed, overallScore } =
        req.body;
      const user = await User.findById(req.user.id);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (retentionSpeed !== undefined) {
        user.preferences.retentionSpeed = retentionSpeed;
      }
      if (memScoreRecalibrationFreq !== undefined) {
        user.preferences
          .memScoreRecalibrationFreq = memScoreRecalibrationFreq;
      }
