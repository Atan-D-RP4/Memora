  ...
  try {
    ...
    if (topics.length === 0) {
      ...
      topics.forEach((topic, index) => {
        ...
        console.log(
          `   User: ${topic.userId ? topic.userId.email : "Unknown"}`,
        );
    ...
    console.log("\n📊 REVISION HISTORY:");
    console.log("==================");
    const revisions = await RevisionHistory.find({}).populate("userId", "email")
    ...
    if (revisions.length === 0) {
      ...
      recentRevisions.forEach((revision, index) => {
        console.log(
          `${index + 1}. Topic: ${
            revision.topicId ? revision.topicId.title : "Unknown"
          }`,
        );
        console.log(
          `   User: ${revision.userId ? revision.userId.email : "Unknown"}`,
        );
